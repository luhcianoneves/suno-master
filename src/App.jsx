...
/* ==================================================================================
   SISTEMA DE INSTRUÇÃO OTIMIZADO (V2)
   ==================================================================================
*/
...
export default function App() {
  ...
  // Para evitar erros de 'import.meta' neste preview, usamos uma string vazia por padrão.
  // No seu projeto local (Vercel), você deve descomentar a linha do import.meta.
  
  // const apiKey = import.meta.env.VITE_GEMINI_API_KEY; // <--- Use esta no Vercel
  const apiKey = ""; // <--- Cole sua chave AQUI para testar no chat agora

  // Injeção de estilos globais (substituindo o index.css para este preview)
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    // Validação para orientar o usuário caso a chave esteja vazia
    if (!apiKey) {
       if (inputText.trim()) {
         setError("⚠️ API Key não detectada. Para testar aqui, edite o código e cole sua chave na variável 'apiKey'. No seu projeto final, use o arquivo .env.");
       }
       return;
    }
    ...
    try {
      ...
      console.error(err);
      setError("Erro ao conectar. Verifique se sua API Key é válida.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 font-sans selection:bg-purple-500/30 pb-20">
